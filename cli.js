...
function getConfig() {
  return new Promise((resolve) => {
    ...
    if (fs.existsSync(configFile)) {
      ...
      const missingConfigFrameIndex = missingConfig.findIndex((configItem) => {
        return configItem.name === "frame";
      });
      if (
        missingConfigFrameIndex > -1 &&
        config["frames"] &&
        config["frames"].length > 0
      ) {
        missingConfig.splice(missingConfigFrameIndex, 1);
      }
      // end frames correction
      ...
      files.forEach((file) => {
        ...
        if (hasSubdirectory) {
          ...
          fs.readdir(subdirectory, (err, files) => {
            if (err) throw err;
            files.forEach((file) =>
              filesToDelete.push(deleteIcon(path.join(subdirectory, file)))
            );
          });
        } else {
          if (file !== "README.md") {
...
// Searches the node tree for nodes of given type
function findChildrenByType(startingNode, type) {
  let result = [];

  if (startingNode.type === type) {
    result.push(startingNode);
  }

  if (startingNode.children?.length) {
    startingNode.children.forEach((child) => {
      result = result.concat(findChildrenByType(child, type));
    });
  }
  ...
  return new Promise((resolve) => {
    ...
    figmaClient
      ...
      .then((res) => {
        ...
        const shouldGetFrames = config.frames && config.frames.length > 0;
        let iconsArray = page.children;

        // If we have the frames config then find by frames
        if (shouldGetFrames) {
          const frameNames = config.frames;
          const components = page.children
            .filter((c) => frameNames.includes(c.name))
            .flatMap((frame) => {
              return findChildrenByType(frame, "COMPONENT_SET");
            });
          iconsArray = components;
        } else if (shouldGetFrame) {
          ...
          deleteIcons().then(async () => {
            ...
            const chunks = chunk(
              ...
              config.maxConcurrentConnections ||
                defaults.maxConcurrentConnections
            );
            let allIcons = [];
            for (let i = 0; i < chunks.length; i++) {
              spinner.start(`Downloading chunks ${i} of ${chunks.length}`);
              const chunkPromise = chunks[i].map((icon) =>
                downloadImage(icon.image, removeFromName(icon.name))
              );
